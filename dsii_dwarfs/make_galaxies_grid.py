...
class SimulateDwarfElliptical:
    def __init__(self,noise_rms=0.02,
                 ...
                 npix=256,distance=10.,logmass=7.,age=12.,feh=-1.5):
        ...
        self.distance,self.logmass,self.age,self.feh = distance,logmass,age,feh
        self.mass = 10.**self.logmass
        self.select_isochrone()
        self.pick_morphology()

        # Pick an isochrone given a mass, and compute flux at 10 pc in each band
    def select_isochrone(self):
        mp = isochrone_picker()
        i = np.array(np.abs(self.feh-mp.metallicities)).argmin()
        closest_feh = mp.metallicities[i]
        if closest_feh < 0:
            mp.isofile = mp.isodir+mp.isofilestring % ('m',-closest_feh)
        else:
            mp.isofile = mp.isodir+mp.isofilestring % ('p',closest_feh)
        isotable = Table.read(mp.isofile,format='ascii.commented_header',header_start=-1)
        logage = np.log10(self.age*1.e9)
        i = np.array(np.abs(logage-isotable['log10_isochrone_age_yr'])).argmin()
        selected_age = isotable['log10_isochrone_age_yr'][i]
        self.isochrone = isotable[np.isclose(isotable['log10_isochrone_age_yr'],selected_age)]
        self.feh = closest_feh
        self.age =  10.**selected_age/1.e9 # Gyr
...
def simulate_HSC(distance,logmass,age,feh):
    sim = SimulateDwarfElliptical()
